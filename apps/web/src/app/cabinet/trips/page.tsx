...
export default async function TripsPage() {
  ...
  return <><SiteHeader /><main><section className="page-hero"><div className="container"><div style={{ display: "flex", justifyContent: "space-between", gap: 25, alignItems: "end" }}><div><span className="eyebrow">Личный кабинет</span><h1 className="page-title">Мои поездки</h1><p className="page-intro">Здравствуйте, {session.name}. Здесь собраны бронирования, оплаты и детали заезда.</p></div><LogoutButton /></div></div></section><section className="section"><div className="container form-stack">{trips.length ? trips.map((trip) => {
    ...
    return <article className="house-card trip-card" key={trip.id}><div className="house-image"><Image src="/images/stitch/asset-021.png" alt={trip.houseName} fill sizes="300px" /></div><div className="house-copy"><span className={`badge ${trip.status.includes("awaiting") ? "badge-warn" : ""}`}>{statusLabels[trip.status]}</span><h3>{trip.houseName}</h3><div className="summary-row"><span>Заезд</span><strong>{trip.checkIn}, после 15:00</strong></div><div className="summary-row"><span>Выезд</span><strong>{trip.checkOut}, до 12:00</strong></div><div className="summary-row"><span>Оплата</span><strong>{formatPrice(Number(trip.paidAmount))} ₽ из {formatPrice(Number(trip.totalAmount))} ₽</strong></div><p>Номер: {trip.publicNumber}</p><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><Link className="button button-ghost" href={`/domiki/${trip.houseSlug}`}>О домике</Link>{due > 0 && !["cancelled","completed"].includes(trip.status) && <Link className="button button-primary" href={`/oplata/${trip.id}`}>Оплатить {formatPrice(due)} ₽</Link>}</div></div></article>;
