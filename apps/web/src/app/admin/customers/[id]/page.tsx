...
export default async function CustomerDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const bookingServiceMap = await getBookingServicesMap(bookingRows.map((booking) => booking.id));
  const paidSummary = calculateCustomerPaidSummary(bookingRows);
  ...
  const general = <><div className="stat-grid"><div className="stat-card"><div className="stat-label">Бронирований</div><div className="stat-value">{bookingRows.length}</div></div><div className="stat-card"><div className="stat-label">Оплачено</div><div className="stat-value">{formatCurrency(paidSummary.total)}</div></div><div className="stat-card"><div className="stat-label">Средний чек</div><div className="stat-value">{formatCurrency(paidSummary.average)}</div></div><div className="stat-card"><div className="stat-label">Любимый домик</div><div className="stat-value stat-value-text">{favoriteHouse}</div></div></div><section className="panel"><h2>Общая информация</h2><div className="summary-row"><span>Имя</span><strong>{customer.firstName} {customer.lastName}</strong></div><div className="summary-row"><span>Телефон</span><strong>{customer.phone}</strong></div><div className="summary-row"><span>Email</span><strong>{customer.email}</strong></div><div className="summary-row"><span>Дата регистрации</span><strong>{customer.createdAt.toLocaleDateString("ru-RU")}</strong></div><div className="summary-row"><span>Последний вход</span><strong>{customer.lastLoginAt?.toLocaleString("ru-RU") ?? "—"}</strong></div><div className="summary-row"><span>Последнее бронирование</span><strong>{bookingRows[0]?.checkIn ?? "—"}</strong></div></section></>;
  const history = <section className="panel"><div className="section-heading compact-heading"><h2>История бронирований</h2><a className="button button-ghost" href={`/api/admin/exports/bookings?format=xls&customerId=${id}`}>Excel</a></div><table className="data-table"><thead><tr><th>Номер</th><th>Дата</th><th>Домик</th><th>Заказанные услуги</th><th>Стоимость</th><th>Статус</th><th>Оплата</th><th>Канал</th><th>Комментарий</th></tr></thead><tbody>{bookingRows.map((booking) => <tr key={booking.id}><td data-label="Номер">{booking.publicNumber}</td><td data-label="Дата">{booking.checkIn} — {booking.checkOut}</td><td data-label="Домик">{booking.houseName}</td><td data-label="Заказанные услуги">{bookingServiceMap.get(booking.id)?.length ? <ul className="compact-list">{bookingServiceMap.get(booking.id)!.map((service) => <li key={service.id}>{service.title}{service.optionTitle ? ` · ${service.optionTitle}` : ""}</li>)}</ul> : "—"}</td><td data-label="Стоимость">{formatCurrency(Number(booking.totalAmount))}</td><td data-label="Статус">{booking.status}</td><td data-label="Оплата">{booking.paymentStatus}</td><td data-label="Канал">{booking.channelName ?? "—"}</td><td data-label="Комментарий">{booking.managerComment ?? "—"}</td></tr>)}</tbody></table></section>;
