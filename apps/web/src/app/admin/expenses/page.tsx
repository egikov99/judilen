...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const conditions = [
    ...
    params.employeeId === "none" ? isNull(expenses.employeeId) : params.employeeId ? eq(expenses.employeeId, params.employeeId) : undefined,
    params.search ? or(ilike(expenses.comment, `%${params.search}%`), ilike(expenseCategories.name, `%${params.search}%`)) : undefined
  ];
  const [rows, categories, houseRows, employeeRows, categoryTotals, houseTotals, employeeTotals, totalRows, monthlyTotals] = await Promise.all([
    db.select({
      id: expenses.id,
    ...
    }).from(expenses).innerJoin(expenseCategories, eq(expenses.expenseCategoryId, expenseCategories.id)).leftJoin(houses, eq(expenses.houseId, houses.id)).leftJoin(employees, eq(expenses.employeeId, employees.id)).leftJoin(users, eq(expenses.createdBy, users.id)).where(and(...conditions)).orderBy(desc(expenses.expenseDate), desc(expenses.createdAt)).limit(1000),
    ...
    db.select({ id: employees.id, name: employees.fullName, total: sql<string>`coalesce(sum(${expenses.amount}), 0)` }).from(expenses).innerJoin(employees, eq(expenses.employeeId, employees.id)).innerJoin(expenseCategories, eq(expenses.expenseCategoryId, expenseCategories.id)).where(and(...conditions)).groupBy(employees.id).orderBy(sql`sum(${expenses.amount}) desc`),
  ...
  return <main className="admin-content"><h1 className="admin-title">Расходы</h1><p className="admin-subtitle">Финансовый учет по домикам, статьям и сотрудникам.</p>
    ...
    <div className="report-grid"><section className="panel"><h2>По категориям</h2>{categoryTotals.map((item) => <div className="summary-row" key={item.name}><span><i className="color-dot" style={{ background: item.color }} />{item.name}</span><strong>{formatCurrency(Number(item.total))}</strong></div>)}</section><section className="panel"><h2>По домикам</h2>{houseTotals.map((item) => <div className="summary-row" key={item.name}><span>{item.name}</span><strong>{formatCurrency(Number(item.total))}</strong></div>)}</section><section className="panel"><h2>По сотрудникам</h2>{employeeTotals.map((item) => <div className="summary-row" key={item.id}><span>{item.name}</span><strong>{formatCurrency(Number(item.total))}</strong></div>)}</section><section className="panel"><h2>По месяцам</h2>{monthlyTotals.map((item) => <div className="summary-row" key={item.month}><span>{item.month}</span><strong>{formatCurrency(Number(item.total))}</strong></div>)}</section></div>
    <ExpenseManager rows={rows.map((row) => ({ ...row, houseId: row.houseId ?? null, houseName: row.houseName ?? null, employeeId: row.employeeId ?? null, employeeName: row.employeeName ?? null, authorName: `${row.authorFirstName ?? ""} ${row.authorLastName ?? ""}`.trim() || "Система" }))} categories={categories} houses={houseRows} employees={employeeRows} canWrite={access.permissions.includes("expenses.write")} exportQuery={exportQuery} />
  </main>;
}
