import { bookingServices, bookings, db, employees, expenseCategories, expenses, houses, salesChannels, services } from "@judilen/db";
...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const expensePeriod = and(gte(expenses.expenseDate, range.from), lte(expenses.expenseDate, range.to), employeeId ? eq(expenses.employeeId, employeeId) : undefined);
  ...
  const [financeRows, bookingRows, cancellationRows, expenseRows, channelRows, popularHouses, popularServices, houseRevenue, houseCosts, cancellationReasons, houseCountRows, employeeOptions, employeeExpenseRows, employeeCategoryRows, reportCategories] = await Promise.all([
    db.select({
    ...
    db.select({ id: employees.id, fullName: employees.fullName }).from(employees).orderBy(asc(employees.fullName)),
    db.select({ id: employees.id, fullName: employees.fullName, total: sql<string>`coalesce(sum(${expenses.amount}), 0)` }).from(expenses).innerJoin(employees, eq(expenses.employeeId, employees.id)).where(expensePeriod).groupBy(employees.id).orderBy(asc(employees.fullName)),
    db.select({ employeeId: employees.id, categoryId: expenseCategories.id, total: sql<string>`coalesce(sum(${expenses.amount}), 0)` }).from(expenses).innerJoin(employees, eq(expenses.employeeId, employees.id)).innerJoin(expenseCategories, eq(expenses.expenseCategoryId, expenseCategories.id)).where(expensePeriod).groupBy(employees.id, expenseCategories.id),
    db.selectDistinct({ id: expenseCategories.id, name: expenseCategories.name }).from(expenses).innerJoin(expenseCategories, eq(expenses.expenseCategoryId, expenseCategories.id)).where(expensePeriod).orderBy(asc(expenseCategories.name))
  ]);
  ...
  if (employeeId) exportQuery.set("employeeId", employeeId);
  const employeeCategoryMap = new Map(employeeCategoryRows.map((row) => [`${row.employeeId}:${row.categoryId}`, Number(row.total)]));
  return <main className="admin-content"><h1 className="admin-title">Отчеты</h1><p className="admin-subtitle">Финансовые и операционные показатели за выбранный период.</p>
    ...
    <section className="panel"><h2>Расходы по сотрудникам и статьям</h2>{employeeExpenseRows.length ? <div className="booking-services-table-wrap"><table className="data-table"><thead><tr><th>Сотрудник</th>{reportCategories.map((category) => <th key={category.id}>{category.name}</th>)}<th>Итого</th></tr></thead><tbody>{employeeExpenseRows.map((employee) => <tr key={employee.id}><td data-label="Сотрудник"><strong>{employee.fullName}</strong></td>{reportCategories.map((category) => <td key={category.id} data-label={category.name}>{formatCurrency(employeeCategoryMap.get(`${employee.id}:${category.id}`) ?? 0)}</td>)}<td data-label="Итого"><strong>{formatCurrency(Number(employee.total))}</strong></td></tr>)}</tbody></table></div> : <p className="notice">Расходов, связанных с сотрудниками, за период нет.</p>}</section>
  </main>;
}
