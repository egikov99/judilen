...
export default async function ContentPage() {
  ...
  return <main className="admin-content"><h1 className="admin-title">Контент и SEO</h1><p className="admin-subtitle">Редактируемые страницы, мета-теги и публикация.</p><section className="panel" style={{ marginBottom: 20 }}><h2>Новая страница</h2><ContentCreateForm /></section><section className="panel"><table className="data-table"><thead><tr><th>Страница</th><th>URL</th><th>SEO title</th><th>Статус</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><strong>{row.title}</strong></td><td>/{row.slug}</td><td>{row.seoTitle}</td><td><span className={`badge ${row.isPublished ? "" : "badge-warn"}`}>{row.isPublished ? "Опубликована" : "Черновик"}</span></td></tr>)}</tbody></table>{!rows.length && <p className="notice">Контентных страниц пока нет.</p>}</section></main>;
}
