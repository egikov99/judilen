...
export default async function AdminBookingsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const defaults = typeof params.houseId === "string" && typeof params.checkIn === "string" && typeof params.checkOut === "string"
    ? { houseId: params.houseId, checkIn: params.checkIn, checkOut: params.checkOut } : null;
  return <main className="admin-content"><h1 className="admin-title">Бронирования</h1><p className="admin-subtitle">Заявки со всех каналов, оплаты и история статусов.</p>{defaults && <QuickBookingForm houses={houseRows} defaults={defaults} />}<section className="panel"><div style={{ display: "flex", justifyContent: "space-between", gap: 20, marginBottom: 20 }}><div className="field" style={{ width: 330 }}><label htmlFor="search">Поиск</label><input id="search" placeholder="Номер, имя или телефон" /></div></div><table className="data-table"><thead><tr><th>Номер</th><th>Гость</th><th>Дом</th><th>Даты</th><th>Сумма</th><th>Статус</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td>{row.publicNumber}</td><td>{row.customerFirstName} {row.customerLastName}</td><td>{row.houseName}</td><td>{row.checkIn} — {row.checkOut}</td><td>{formatCurrency(Number(row.totalAmount))}</td><td><BookingStatusControl id={row.id} status={row.status} /></td></tr>)}</tbody></table>{!rows.length && <p className="notice">Бронирований пока нет.</p>}</section></main>;
}
