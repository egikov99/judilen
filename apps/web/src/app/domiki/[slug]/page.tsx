  ...
  return (
    <PublicShell>
      ...
      <section className="section" style={{ paddingTop: 45 }}><div className="container">
        <div className="gallery">{house.images.slice(0,3).map((image, index) => <PublicImage key={image} src={image} context={`house-gallery:${house.id}:${index}`} alt={`${house.name}, фото ${index + 1}`} width={900} height={700} priority={index === 0} loading={index === 0 ? "eager" : "lazy"} />)}</div>
