    ...
    <PublicShell>
      ...
      <section className="section"><div className="container">
        {services.length ? <div className="service-grid">{services.map((service) => {
          ...
          return <article className="service-card" key={service.id}><PublicImage src={service.images[0] ?? DEFAULT_IMAGE_URL} context={`service-card:${service.id}`} alt={service.title} width={512} height={512} loading="lazy" /><div><span className="eyebrow">от {formatCurrency(price)} {servicePriceUnitLabel(service)}</span><h2>{service.title}</h2><p>{service.description}</p>{(service.minRentalHours || service.extensionPrice !== null) && <div className="service-rental-meta">{service.minRentalHours && <span>Минимальный срок аренды: {service.minRentalHours} часа</span>}{service.extensionPrice !== null && <span>Продление: {formatCurrency(service.extensionPrice)} / час</span>}</div>}{!!service.options.length && <div className="form-stack">{service.options.map((option) => <div className="summary-row" key={option.id}><span>{option.title}</span><strong>{formatCurrency(option.price)}</strong></div>)}</div>}<Link className="text-link" href={`/uslugi/${service.slug}`}>Подробнее →</Link></div></article>;
