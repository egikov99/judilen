...
export function ServiceManager({ services, options, houses, serviceHouseIds, permissions }: {
  ...
  return <div className="form-stack">
    ...
    <section className="panel">
      <table className="data-table"><thead><tr><th>Услуга</th><th>Цена</th><th>Фото</th><th>Варианты</th><th>Публикация</th>{(canUpdate || canDelete) && <th>Действия</th>}</tr></thead><tbody>{services.map((service) => <tr key={service.id}><td data-label="Услуга"><strong>{service.title}</strong><br /><small>/{service.slug}</small></td><td data-label="Цена">{formatCurrency(Number(service.basePrice))}</td><td data-label="Фото">{service.images.length}</td><td data-label="Варианты">{options.filter((option) => option.serviceId === service.id).length}</td><td data-label="Публикация"><span className={`badge ${service.isActive ? "" : "badge-warn"}`}>{service.isActive ? "Активна" : "Скрыта"}</span></td>{(canUpdate || canDelete) && <td data-label=""><div className="action-row">{canUpdate && <button className="button button-ghost" onClick={() => setSelected(service)}>Редактировать</button>}{canDelete && <button className="button button-ghost" disabled={deletingId === service.id} onClick={() => remove(service)}>{deletingId === service.id ? "Удаление…" : "Удалить"}</button>}</div></td>}</tr>)}</tbody></table>
